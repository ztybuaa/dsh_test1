 ...
 * （照抄 `state.json` 的启动同步就是一次）都会在**没人碰过按钮**的情况下改这一格的缩放；在
 * 票 #19 那会儿它还会把"人工接管"那个模式一并设上，自动适配于是再也不动 —— 本票重新打开时的
 * 现象正是那个（启动瞬间就是 `manual`、`fitPasses` 一直是 0）。
 *
 ...
 * 那是**唯一**还允许走缺省的路。
 *
 * **票 #20b：`mode` 的语义去掉了，这个标签与两个字段都留着。** 外壳不再有模式状态机（适配永远
 * 开着），但这条协议一个字节都不能改：旧插件还在发这种形状，票面明令"解析必须继续容忍它、
 * 兼容不许破"。`mode: 'manual'` / 缺省 = "把 `zoom` 按上去"，`mode: 'auto'` = "现在就重新
 * 适配一次"。见 `shell/spaces.js` 里 {@link ZOOM_COMMAND_KIND} 那一段。
 */
