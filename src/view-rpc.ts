 ...
 * 面板可以请求的每一个动作。
 *
 * 四个导航/生命周期动作 + 四个缩放动作 + 一个**重新适配**的动作（票 #19 的「自动」），加上一个
 * 纯读的 `state`：值域刻意小而封闭 —— 面板是网页，它的输入是**不可信**的，所以这里没有"带参数的
 * 通用命令"，每一个动作能做的事都在这一行里看得见。
 *
 * `auto` 是票 #19 加的那一个，当时的意义是**唯一**能让人从"手动缩放"回到"自动适配"的动作。
 * **票 #20b 把那个模式删掉了**：适配永远开着，一次手动缩放只是"现在的值"，下一次几何变化或换页
 * 就会被重新适配。于是这个动作不再切换任何状态，它剩的意思是"**别等下一次变化，现在就重新适配
 * 一次**"，而它继续留在表里只是因为旧客户端还在调它（票面：兼容不许破）。面板不再渲染它。
 *
...
export const VIEW_ACTIONS = [
  ...
  'zoom-reset',
  'zoom-to',
  // 票 #20b：面板上那颗「自动」按钮已经删掉（适配永远开着，没有模式可以交还），但**这个端点留着**
  // —— 旧客户端仍在调它，而它的语义今天仍然说得通："别等下一次几何变化，现在就重新适配一次"。
  'auto',
  'state',
] as const
