...
| 同上，再确认**页面还是原来那一页**（不是被重开成空白页） | 页面内容与折叠前一致 | 👤 这条只有"矩形与可见性"被自动化；**"内容还是原来那一页"请你自己看一眼**。想主动清空就用工具条上的「**重新开始**」（回到初始页并重置缩放） |
...
| **后退 / 前进 / 刷新** | 「后退一页」「刷新」→ 真的退/真的刷；没有历史时**说得出是哪一类** | ✅ `tests/view-actions.spec.ts`、`tests/history-truth.spec.ts` › *「能不能后退」是问浏览器引擎的*（不是插件自己记的账）；面板上还有一排按钮（见 §13） |
...
## 11. 那一格上的控件(§13)与两条"曾经没有、现在有了"的能力

**这一节原本记的是"PRD 已要求但本期没做"** —— 那是当时的事实,**现在都做完了**。留在这里是为了让读过旧版本的人知道结论变了,以及去哪儿看证据。
