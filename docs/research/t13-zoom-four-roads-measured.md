...
> **本文的结论已被推翻两次，最终结论见 `t13-zoom-out-measured.md`（保留本文只作记录）。**
>
> - 原文把"布局视口变小"当成了"内容被缩小"，于是得出"路 C 能让 1200px 塞进 620px"；
> - 文末那次"更正"又走向另一个极端：说"插件够得到的任何手段都做不到"，而它**只量了放大方向**
>   （1.94 / 1.5 / 3 倍）与移动端模拟，根本没有量用户要的**缩小**；
> - 最终的实测（真窗口像素 + 仪器自检）是：**CDP 那条路（路 C）在缩小方向上不但不成立，
>   还是负功能**（它把横向溢出弄没了，于是连滚动都够不到最右端），**外壳侧的
>   `webContents.setZoomFactor()` 成立**（zoom=0.5 时蓝条高度减半、红标出现在半尺寸位置上）。
>
> 下面各节保留原始读数（它们本身是对的），结论一律以上面那一份为准。
