...
### 3.1 这份验收是在哪棵树上跑的（以及我自己漏过一次）

上面 7 次跑完之后，我发现自己新加的那一组故障注入用例**每跑一次就漏一个档案目录**：
两条用例各起一块外壳，而组里只用一个变量记住"最近那块"，于是第一条一被赋值覆盖，那块外壳
就没人停了 —— 结果是 %TEMP% 里多出一个 156 个文件的档案目录，而且漏得**不声不响**
（没人调用清理，自然也没有"删不掉"的警告）。

修法：把它们收进一个列表，`afterAll` 里**每一块**都 `stop()`。之后在**最终那棵树**上又跑了
**5 次连续**（`exit=0`、9 文件 / 82 用例），并且跑前跑后对全部 15 个改动文件做 SHA256 比对：
**一致**（`TREE_UNCHANGED`）。

这条修的是**测试自己的卫生**，不是被测行为；记在这里是因为它属于同一个主题 ——
"清理是家务，但家务漏了也得有人发现"。发现它的方式不是测试（套件照样全绿），而是跑完去看 %TEMP%。
