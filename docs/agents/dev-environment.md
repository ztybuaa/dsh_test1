...
**重试的正确姿势（一次实测的时间线，2026-09-15 深夜）**：连续 **9 次** `git push` 全部 `exit=128` / `Failed to connect to github.com port 443 after ~21s`，其间 `gh api user` 一直是通的（建票、发评论都正常）。挂一个**每 60 秒重试一次的后台循环**，**第 5 次（约 4 分钟后）成功**。所以：

- 失败时**照常做别的**（本地 commit、写文档、跑测试都不受影响），把 `git push` 丢进后台循环；
- **别去"修"它**：不是凭证、不是 remote、不是仓库状态；
- 顺带量到的一条：本机 `HKCU\...\Internet Settings` 里的代理是 `127.0.0.1:7897` 但 **`ProxyEnable=0x0` 且该端口没有监听**，所以 `git -c http.proxy=http://127.0.0.1:7897` **也不通**——此路不用再试。
