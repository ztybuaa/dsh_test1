...
- **同一文档内的 DOM 变动会让 `ref` 错位。** 上面记的文档身份只防"换了文档"。在**同一文档内**（无导航）列表重排或重渲染之后，`ref` 在动作时按选择器引擎的第 N 个匹配重新解析，**可能落到另一个元素上**，而且**不会被拒绝**。SPA／前端框架重渲染的页面正属于这一类。当前语义是"既不一定失效、也不保证正确"——**不要在文档或错误文案里把它说成"ref 一直有效"**。
...
- **滚动之后的 bounds 未经验证**：已确证 bounds 是视口坐标（视口外元素的 `y` 远大于视口高，且与页面自身几何严格相等），但**滚动页面后再读一次**没有被测过。
