 ...
 *
 * 在**命令**里它的意思是"把这一格的缩放设成 1"，在**状态**里它的意思只是"这一格现在是 100%"。
 * 谁把外壳发布的那条记录原样写回 `request.json`（一次状态回显、一个照抄 `state.json` 的启动
 * 同步），谁就在**没有任何人碰过按钮**的情况下改了这一格的缩放 —— 在票 #19 那会儿后果还要
 * 严重得多：它会把"人工接管"这个模式一并设上，于是自动适配再也不动（"启动瞬间变成 manual、
 * `fitPasses` 一直是 0"那个现象）。
 *
 * **票 #20b：那个模式没有了，但这条标签留着。** 理由没有随模式一起消失：一条状态回显仍然
 * 不该被当成命令（它会按上一个没人要的值、还会作废一轮正在跑的适配）。所以：**新客户端发缩放
 * 命令时必须带这个标签**，`zoom`/`mode` 只在标签下面才有"命令"的含义。旧插件那个不带标签的
 * 形状**照旧按命令解释**（它写 request.json 从来只为了发命令，见 {@link parseRequest} 里那条
 * 兼容路径），兼容语义一个字节都不变。
 */
 ...
 *
 *  - **带标签**（新客户端，票 #19 重开之后）：
 *      `{name, kind: 'zoom', zoom: 0.9, mode: 'manual'}` —— 把缩放设成 0.9（票 #20b 之前还
 *      意味着"从此由人管"；**那层语义已经删掉了**，见下面那段）；
 *      `{name, kind: 'zoom', mode: 'auto'}`             —— 现在就重新适配一次（票 #19 时是
 *      "把这一格交回自动适配"，#20b 之后适配永远开着，所以它只剩"别等下一次几何变化"这个意思）。
 ...
 *
 * **票 #20b：`mode` 这个字段的语义去掉了，解析一个字都没改。** 解析这一层继续接受 `mode`
 * 与 `kind: 'zoom'`（旧插件还在发；票面原话"解析必须继续容忍它，兼容不许破"），而外壳**不再**
 * 因为它切换任何状态：`manual`/缺省 = "把 `zoom` 按上去"，`auto` = "现在重新适配一次"。
 * 也就是说 `mode` 现在只是"这条命令怎么读"的一个词，不是"这块视图从此归谁管"。
 *
