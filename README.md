...
### 那一格上的界面(T20):地址栏、标签标题、档位

五处补齐,都在**面板里**(不在被驱动的那一页上,所以一个字节都进不了快照):

| 是什么 | 怎么用 |
|---|---|
| **地址栏** | 输入网址**回车**打开;跳转之后框里的地址**跟着页面走**(读回来,不是自己记的) |
| **标签标题** | 侧边栏那个标签写的是**当前页面标题**,读不到回落「浏览器」 |
| **读数** | 只有"模式 + 百分比"(必要时加 `加载中` / `✗ 为什么没成`);诊断话术搬到了 tooltip 与 `data-dsh-view-diagnostic` 上,一个字都没删 |
| **档位** | 点那个百分比给出一排标准档位(50/67/75/80/90/100/110/125/150/175/200),`−`/`+`/重置/自动 全部保留 |
| **加载与悬停** | 页面还在加载时读数上写 `加载中`;悬停 `←`/`→` 显示**引擎历史里那一页**的标题 |

**地址栏补协议的规则**(输入框的 `title` 上也写着这条):

| 你输的 | 去哪儿 | 为什么 |
|---|---|---|
| `example.com` | `https://example.com` | 只写主机名 ⇒ 补 `https://` |
| `localhost:3000` / `127.0.0.1:8080` / `[::1]:5173` | `http://…` | **唯一的例外**:本机上跑着的东西几乎从不带 TLS |
| `http://…` `https://…` `file:///…` `about:…` | 原样 | 写明了就用它 |
| `javascript:` `data:` `mailto:` `blob:` | **拒**,并说清 | 一条说不清的地址比"这个我不开"更容易骗人 |
| 带空格的、空的 | **拒**,并说清 | 这一格**不搜索**,不许把一句话悄悄变成一次搜索 |

**"按一下要等一秒"没了**(票 #20 E)。实测那 0.9 秒不是 HTTP、不是外壳发布(4–8 ms,
其中 `cookies.get({})` 0–1 ms)、也不是轮询(150 ms),而是会话在缩放之后**等一个合成帧** ——
而那一页不被合成时 500 ms 里只出 0–1 帧。现在改成"一次同步强制布局 + 有界核对",
同一次按 **119–175 ms**。完整拆解、被否掉的那条怀疑、以及没验到的部分:
[`docs/research/t20-why-the-panel-button-waits-a-second.md`](docs/research/t20-why-the-panel-button-waits-a-second.md)、
决定见 [`docs/adr/0015-toolbar-polish-and-the-frame-wait.md`](docs/adr/0015-toolbar-polish-and-the-frame-wait.md)。
